...
metalsData=read.csv(file="metalsData.csv")
...
pct_meHg_flat=melt(tmeHg_mat,id.vars=c("Lab_Name","Sample_Tag","Project_ID","SiteID","Date","WaterColLoc","Matrix","Lab_ID","Year"),value.name="Result",variable.name="Parameter")
  ...
  # Sidebar layout with input and output definitions ----
    ...
    radioButtons(inputId = "analysisType",
		...
		conditionalPanel(
			...
			helpText("This portion of the tool allows comparison of nutrient and chlorophyll a relationships between GSL and two EPA national monitoring programs: the National Lakes Assessment (NLA, 2012) and the National Coastal Condition Assessment (NCCA, 2010)."),
		...
		conditionalPanel(
			...
			helpText("This portion of the tool estimates DBL, UBL, and total nutrient masses in Gilbert Bay following Naftz (2017) with small modifications. DBL and UBL volumes (and therefore constituent masses) outside 2011-2014 are extrapolated from models presented in Naftz (2017). Bear River and Farmington Bays and inflows currently excluded from these analyses."),
...
conditionalPanel(
  ...
  # Sidebar layout with input and output definitions ----
	...
	conditionalPanel(condition="input.metalsPlotType==2",
		...
		#numericInput("metals_bp_h","Benchmark",value=0),
...
output$plot1=renderPlot({
	...
	if(input$analysisType==3){
		...
		if(input$mass1=="FN"){abline(h=3.2e7,col="purple",lwd=2)}
...
if(input$metalsPlotType==1){
	...
	par(mfrow=c(3,1),mar=c(6.1,6.1,2.1,2.1))
	if(dim(metalsData_plots[metalsData_plots$Parameter==input$metalsTS_param1&metalsData_plots$Matrix==matrices[1],])[1]>0){
		...
		points(Result~Date,plotData1[plotData1$WaterColLoc=="BIO",],col="Green",pch=19,cex=2.5)
	...
	if(dim(metalsData_plots[metalsData_plots$Parameter==input$metalsTS_param2&metalsData_plots$Matrix==matrices[2],])[1]>0){
		...
		points(Result~Date,plotData2[plotData2$WaterColLoc=="BIO",],col="Green",pch=19,cex=2.5)
	...
	if(dim(metalsData_plots[metalsData_plots$Parameter==input$metalsTS_param3&metalsData_plots$Matrix==matrices[3],])[1]>0){
		...
		points(Result~Date,plotData3[plotData3$WaterColLoc=="BIO",],col="Purple",pch=19,cex=2.5)
...
output$plot3=renderPlot({
	...
	metalsData_plots=(metalsData_plots)
		...
		ymax=max(bp_plotdata$Result)
		if(input$log_metalsBP=="TRUE"){ymin=min(bp_plotdata$Result)*0.5}else{ymin=0}
		#if(ymax<input$metals_bp_h){ymax=input$metals_bp_h*1.1}
		ylabel=paste0(input$metalsBP_param," ",unique(bp_plotdata$Units))
		if(dim(bp_plotdata)[1]>0){boxplot(bp_plotdata$Result~bp_plotdata[,input$metalsBP_aggby],log=logybp,ylab=ylabel,cex.lab=1.25,cex.axis=1.25,ylim=c(ymin,ymax))}
		#abline(h=input$metals_bp_h,col="green",lwd=3)
	}
})
